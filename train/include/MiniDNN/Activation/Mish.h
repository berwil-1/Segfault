...
public:
    ...
    static inline void
    activate(const Matrix & Z, Matrix & A) {
    ...
    static inline void
    apply_jacobian(const Matrix & Z, const Matrix & A, const Matrix & F, Matrix & G) {
        ...
        const auto mask = Z.array().cwiseEqual(Scalar(0));
        G.noalias() = mask.select(Scalar(0.6), A.cwiseQuotient(Z));
