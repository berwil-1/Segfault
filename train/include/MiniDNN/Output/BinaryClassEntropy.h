...
public:
    void
    check_target_data(const Matrix & target) {
    ...
    void
    check_target_data(const IntegerVector & target) {
        // Each element should be either 0 or 1
    ...
    void
    evaluate(const Matrix & prev_layer_data, const Matrix & target) {
        // Check dimension
        ...
        m_din.array() = (target.array() < Scalar(0.5))
                            .select((Scalar(1) - prev_layer_data.array()).cwiseInverse(),
                                    -prev_layer_data.cwiseInverse());
    }

    void
    evaluate(const Matrix & prev_layer_data, const IntegerVector & target) {
        // Only when the last hidden layer has only one unit can we use this version
        const int nvar = prev_layer_data.rows();

        if (nvar != 1) {
            throw std::invalid_argument("[class BinaryClassEntropy]: Only one response variable is "
                                        "allowed when class labels are used as target data");
        }
        ...
        // Same as above
        m_din.resize(1, nobs);
        const auto t0 = target.array().cwiseEqual(0);

        m_din.array() = t0.select((Scalar(1) - prev_layer_data.array()).cwiseInverse(),
                                  -prev_layer_data.cwiseInverse());
    }
