    ...
    int max_fen_repeats)
{
    ...
    while (offsets.size() < max_samples) {
        ...
        if (cnt >= max_fen_repeats) continue;

        Board board = Board::fromFen(fen);
        auto& ccnt = clock_count[board.fullMoveNumber()];
        if (ccnt >= 100000) continue;
...
struct FenEvalDataset : torch::data::datasets::Dataset<FenEvalDataset> {
    ...
    torch::data::Example<> get(size_t index) override {
        ...
        if (!file.is_open() || opened_path != path_) {
            ...
            if (!file) throw std::runtime_error("Failed to open " + path_);
            opened_path = path_;
        }
        ...
        auto x = torch::from_blob(
            (void*)enc.data(),
            {static_cast<int64_t>(board_size)},
...
main() {
    ...
    if (offsets.empty()) throw std::runtime_error("No samples indexed.");
